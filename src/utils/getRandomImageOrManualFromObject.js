...
export const getRandomImageOrManualFromObject = (object) => {
  ...
  const manualImageKeys = Object.keys(object).filter(key => key.includes("MANUAL_IMG") || key.includes("MANUAL"));
  const manualImageValues = manualImageKeys.map(key => object[key]);


  // get only nonempty string from manualImageValues
  const nonEmptyManualImageValues = manualImageValues.filter(value => value !== "");


  return nonEmptyManualImageValues[Math.floor(Math.random() * nonEmptyManualImageValues.length)];
}
