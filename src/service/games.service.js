...
export function getCurrentGame(req, res, next) {
  ...
  if (timeDiff > 1000 * 130) {
    ...
    currentServerGameData.mainImageUrl =
      currentServerGameData.currentRecipe.ATT_FILE_NO_MAIN;
  ...
  if (1000 * 0 <= timeDiff && timeDiff < 1000 * 40) {
    ...
    const recipeDataList = recipeData;
    randomInt = Math.floor(Math.random() * 1000);
    currentServerGameData.currentRecipe =
      currentServerGameData.currentRecipe ?? recipeDataList[randomInt];
    console.log(
      'currentServerGameData',
      currentServerGameData?.currentRecipe?.RCP_NM
    );
    ...
    currentServerGameData.answerBlankData =
      currentServerGameData.answerBlankData ?? answerBlankData;

    const recipeIndex =
      currentServerGameData.recipeIndex ?? Math.floor(Math.random() * 1000);
    currentServerGameData.recipeIndex = recipeIndex;

    // currentRecipe = recipeDataListFile[currentServerGameData.recipeIndex];

    const initialCompleteFoodImage =
      currentServerGameData.currentRecipe.ATT_FILE_NO_MAIN;
  ...
  if (1000 * 40 <= timeDiff && timeDiff < 1000 * 80) {
    ...
    const recipeStepImageList = getRecipeStepImageListFromObject(
      currentServerGameData?.currentRecipe
    );
  ...
  if (1000 * 80 <= timeDiff && timeDiff < 1000 * 120) {
    ...
    currentServerGameData.hint2 =
      currentServerGameData.currentRecipe.RCP_PARTS_DTLS;
  ...
  try {
    ...
    res.json({
      data: { ...result, debugTimeDiff: timeDiff / 1000 },
      message: 'success',
    });
